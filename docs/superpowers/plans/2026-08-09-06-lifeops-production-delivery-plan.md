...
**Approved unfamiliar-cluster manual acceptance atoms:**
- Architecture and terminology: explain that LifeOps builds only `lifeops-web` and `lifeops-api`, that the migration Job reuses the API image, and that the Web image's static Nginx, the Gateway API specification and Envoy Gateway or NGINX Gateway Fabric controllers are separate responsibilities; explain the embedded single-instance MySQL boundary and why Redis is absent until an evidence-backed ADR defines consistency, failure and rollback behavior.
- Capability-first preflight: give user-run read-only checks for Kubernetes version and CPU architecture, nodes, Gateway API or Ingress, LoadBalancer or MetalLB or external load balancer, DNS/TLS, dynamic StorageClass provisioning, RWO/RWX, Secret or ExternalSecret, monitoring and registry pull access; state the expected result, failure meaning and selection branch for every check.
- Cluster-operation boundary: separate platform prerequisites from LifeOps delivery, never require the executor to read kubeconfig or run `kubectl`, Helm install/upgrade, Argo sync/rollback or cluster smoke, and label those deployment actions as commands the user runs in their own authorized terminal.
- Secret handoff: cover imagePullSecret plus application/database Secret or ExternalSecret choices without embedding plaintext, asking the user to paste credentials into chat, or committing generated Secret values.
- Database branches: document embedded official MySQL `8.4.10` single-replica StatefulSet plus RWO, external managed RDS, binary MySQL on an independent VM rather than a worker node, and user-managed HA/Operator MySQL; each branch covers Secret/TLS, network, backup/restore, upgrade, connection limits, failure boundary and migration away from the embedded instance, without calling the embedded topology highly available.
- Media-storage branches: require RWX for multi-replica API filesystem media, prefer S3-compatible object storage when an unfamiliar cluster lacks RWX, allow single-replica API plus RWO only for bounded small/risk-accepted use and never bypass the chart rejection of multi-replica API plus non-RWX filesystem media.
- Entry-controller branches: recommend Gateway API plus Envoy Gateway, map NGINX Gateway Fabric adaptation to controller, parentRef, namespace, policy and values changes without rebuilding images, and provide a chart-backed Ingress fallback; explain the distinct roles of LoadBalancer, MetalLB, external load balancer, DNS and TLS.
- Immutable release inputs: identify the exact Web/API immutable digests from the release manifest and production values, reject tags or local image IDs as substitutes, and keep user-specific namespace, hostname, storage and platform endpoints explicit.
- Offline delivery preflight: verify documentation links, examples, values schema, Helm lint and both default/production renders before any user deployment command, with no undocumented local path or credential dependency.
- Deployment paths and order: provide Argo CD as the recommended user-operated path and Helm as the alternative, then explain migration Job, Web/API and optional MySQL ordering plus HTTPRoute/Ingress, DNS and TLS checks.
- User application smoke: cover health, login, authenticated read/write, restart persistence, publishing/media where enabled, exact revision/digest and an exactly-once Life sentinel; cleanup is limited to uniquely prefixed application records created by the smoke and never deletes Pods, PVCs, namespaces or unrelated user data.
- Operations handoff: cover monitoring, logs, alerting, truthful optional-integration states, database/media backup and verified restore without fabricating unavailable platform facts.
- Upgrade and rollback: use immutable digest-only changes, verified user-owned backups, forward-compatible migrations and an explicit maintenance decision for disruptive rollback; prohibit destructive down migration and automated access to the user's cluster or database.
- Scaling guidance: explain Web/API horizontal and vertical scaling, API HPA min `2`/max `6`, requests/limits, PDB, anti-affinity/topology spread, Gateway capacity, database connections/performance and media throughput; use `API maximum database connections = API maximum replicas × per-replica connection limit + migration/admin headroom`, avoid invented hardware or QPS, and state when and how to validate pod/resource, database, S3 and external-MySQL changes without breaking sessions, idempotency, media or migration safety.
- Command safety: every command uses placeholders, names the machine and directory, states expected output, failure meaning and safe fallback, and never places credentials, private endpoints or kubeconfig content in chat, Git or evidence.
- Repository asset mapping: map every documented branch to actual chart values, templates and resources including Deployment, Service, HTTPRoute/Ingress, StatefulSet, Job, PVC, NetworkPolicy, PDB, HPA and ExternalSecret, and validate the mapping with link checks, lint/render/schema, example configuration and the user smoke checklist.
- Ordered manual structure: the final manual contains, in order, terminology/images/architecture; capability matrix and decision tree; platform prerequisites; pull/application/database secrets; database/media/entry branches; immutable digests; Helm preflight; Argo or Helm deployment; workload order; routing/DNS/TLS; health/login/write/restart/exact-digest smoke; observability/logs/alerts/backup; upgrade/rollback/troubleshooting; and scaling/component migration.

- [x] **Step 1: Formalize and write failing package and user-smoke tests.** First synchronize the approved unfamiliar-cluster atoms into the source registry, acceptance matrix and traceability. Then test missing files/digests/checksums, deployable placeholders, Secret values, unsafe filesystem replica/storage combinations, public `/metrics`, overbroad RBAC, wrong expected revision/digest, failed health/auth/persistence/life sentinel, cleanup outside smoke-created application records, required manual sections, branch-to-chart mappings and command safety metadata.

- [x] **Step 2: Implement the offline deployment-package validator.** Check Helm lint/template, values schema, immutable images, migration order, health probes, resource/security contexts, Secret refs, RBAC/network policies, monitoring resources, runbook anchors and release-manifest hashes.

- [x] **Step 3: Implement the user-run application smoke.** Reuse remote Playwright journeys for public/auth/private, CRUD, media, publishing and the exact-once life transaction sentinel. Cleanup uses documented application APIs and a unique test prefix; it does not delete Pods, PVCs, namespaces or user data.

- [x] **Step 4: Write the deployment checklist.** Cover UHub pull Secret creation by the user, database Secret, namespace, storage/media choice, Gateway/Ingress, DNS/TLS, Helm render/install or Argo Application adoption, migration completion, readiness, expected digest checks and smoke invocation without embedding real credentials.

- [x] **Step 5: Write safe backup/restore and rollback procedures.** Require a user-owned verified database backup before upgrades, digest-only GitOps revision changes, forward-compatible migrations and a maintenance decision for disruptive rollback. Do not automate access to the user's cluster or database.

- [x] **Step 6: Document platform verification.** Provide Prometheus target/query, Grafana dashboard, PrometheusRule, Elasticsearch/Kibana request-ID and Platform Center deep-link checks the user can perform after installing those components. Disabled integrations remain truthful and do not block Web delivery.

- [x] **Step 7: Perform an offline acceptance drill.** Starting only from the release manifest and docs, follow every non-cluster step, render both Helm/Argo paths, resolve all links/commands, confirm every user input is named and verify no step depends on an undocumented local path or credential.

- [x] **Step 8: Run the final package validator and record the sanitized summary.** Any skipped required package check keeps DELIVERY-01 open.
...
- [x] **Step 9: Commit or hash P6-T7** with message `docs(deploy): deliver user-operated Kubernetes handoff`. Do not include kubeconfig, credentials, private endpoints or user data.
