...
test('real evidence manifest is current and preserves its atom-derived rollups', async () => {
  ...
  assert.deepEqual(issues, [])
  assert.equal(manifest.evidence.length, 462)
  assert.equal([...atomStatuses.values()].filter((status) => status === 'verified-local').length, 936)
  assert.equal([...atomStatuses.values()].filter((status) => status === 'partial').length, 384)
  assert.equal([...atomStatuses.values()].filter((status) => status === 'pending').length, 107)
