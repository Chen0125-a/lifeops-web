...
最终发布在 Chromium、Firefox 和 WebKit 上验证。公开页覆盖 1440×900、1024×768、768×1024、390×844、320 CSS px 与 200% zoom；私人路由、键盘焦点、触控目标和 serious/critical WCAG 扫描同样覆盖必要断点。

公开星盘始终是四个完整同心圆环和五个语义对象；四圈在 normal motion 下持续自然转动，暂停、文档隐藏和 `prefers-reduced-motion` 使用同一个可感知状态。中心只显示 `05 / 此刻正在发生`，不得恢复发光球。登录在桌面让星盘自然左移、标题淡化后退、任务窗位于右侧；390px 使用全屏任务层。night 登录保持深色，day 登录保持兼容的明亮表面。

当前性能合同保持原预算：主题切换和连续运动在官方 Linux 浏览器门禁中通过，不通过降低帧采样、放宽阈值、增加 retry 或减慢动效获得绿灯。最终八张 day/night、rest/login 图由主执行者逐张打开复核，截图生成本身不等于视觉通过。
